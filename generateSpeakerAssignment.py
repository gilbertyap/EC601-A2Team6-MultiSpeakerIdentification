...
# This process uses the most aggressive setting of webrtcvad with a 10ms window
# to detect voice in an audio file. The user is then responsible for giving
# an identity to each audio clip.

import argparse, contextlib, os, simpleaudio, sys, webrtcvad, wave
...
if __name__ == '__main__':
    ...
    fileBytes = getAudioFileData(filePath)
    # Many cases for file paths
    ...
    with open(fileName+'.rttm','w') as f:
        ...
        print('Generating reference rttm for {}...'.format(fileName))
    ...
    print('Finished with file \'{}\''.format(fileName))